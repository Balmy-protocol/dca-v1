...
async function main() {
  ...
  console.time('Verifying zrx swapper ...');
  const zrxSwapper = await deployments.getOrNull('DCAZRXSwapper');
  await run('verify:verify', {
    address: zrxSwapper!.address,
    constructorArguments: zrxSwapper!.args,
    contract: 'contracts/DCASwapper/DCAZRXSwapper.sol:DCAZRXSwapper',
  });
  console.timeEnd('Verifying zrx swapper ...');
}
