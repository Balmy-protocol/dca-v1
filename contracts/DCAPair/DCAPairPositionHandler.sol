...
abstract contract DCAPairPositionHandler is DCAPairParameters, IDCAPairPositionHandler, ERC721 {
  ...
  function _modifyRate(uint256 _dcaId, uint192 _newRate) internal {
    _assertPositionExistsAndCanBeOperatedByCaller(_dcaId);

    DCA memory _userDCA = userPositions[_dcaId];

    uint32 _swapsLeft = _userDCA.lastSwap - performedSwaps;
  ...
  function _modifySwaps(uint256 _dcaId, uint32 _newSwaps) internal {
    _assertPositionExistsAndCanBeOperatedByCaller(_dcaId);
  ...
  function _addFundsToPosition(
    ...
    uint192 _newRate = uint192(_total / _newSwaps);
